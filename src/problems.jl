...
An instance of the robust PCA problem `A = L + S`, where `L` is a low-rank
matrix and `S` is a sparse matrix. Stores the vectorized version of each
matrix and size information.
"""
struct RobustPCAProblem
  Avec::Vector{Float64}
  Lvec::Vector{Float64}
  Svec::SparseVector{Float64, Int64}
  r::Int      # Rank of L.
  m::Int      # Number of rows of A.
  n::Int      # Number of columns of A.
  α::Float64  # Row and column sparsity factor.
end
...
"""
  robust_pca_problem(m::Int, n::Int, r::Int, p::Float64)

Create a random robust PCA problem.
"""
...
"""
  subgradient_sparse(problem::RobustPCAProblem, S::Matrix{Float64})

Compute a subgradient of the distance of `S` to the set of sparse
matrices.
"""
function subgradient_sparse(
  problem::RobustPCAProblem,
  Svec::AbstractVector{Float64},
)
  Sdiff = Svec - proj_sparse!(problem, Svec[:])
  ds = norm(Sdiff)
  return (ds ≤ 1e-15) ? zero(Svec) : Sdiff / ds
end

function dist_sparse(
  problem::RobustPCAProblem,
  Svec::AbstractVector{Float64},
)
  return norm(Svec - proj_sparse!(problem, Svec[:]))
end

"""
  proj_lowrank(problem::RobustPCAProblem, Lvec::AbstractVector{Float64})

Project the `L` solution of a robust PCA problem to the set of rank-`r`
matrices, where `r` is the rank of the ground truth.
"""
function proj_lowrank(
  problem::RobustPCAProblem,
  Lvec::AbstractVector{Float64},
)
  m = problem.m
  n = problem.n
  r = problem.r
  # Compute SVD of reshaped matrix and keep largest `r` components.
  svd_obj, _ = svds(
    reshape(view(Lvec, :), m, n),
    nsv = r,
    ritzvec = true,
  )
  return vec(svd_obj.U * (svd_obj.S .* svd_obj.Vt))
end

"""
  subgradient_lowrank(problem::RobustPCAProblem, L::Matrix{Float64})

Compute a subgradient of the distance to the set of low-rank matrices.
"""
function subgradient_lowrank(
  problem::RobustPCAProblem,
  Lvec::AbstractVector{Float64},
)
  Ldiff = Lvec - proj_lowrank(problem, Lvec)
  ds = norm(Ldiff)
  return (ds ≤ 1e-15) ? zero(Ldiff) : Ldiff / ds
end

function dist_lowrank(
  problem::RobustPCAProblem,
  Lvec::AbstractVector{Float64},
)
  return norm(Lvec - proj_lowrank(problem, Lvec))
end

"""
  proj_range(problem::RobustPCAProblem, Lvec::AbstractVector{Float64}, Svec::AbstractVector{Float64})

Project the pair `(Lvec, Svec)` to the set:

  (L, S) : L + S = problem.A.

Return a vector containing the vertically stacked projections of `Lvec` and
`Svec`, respectively.
"""
function proj_range(
  problem::RobustPCAProblem,
  ...
  return [
    0.5 * (Lvec - Svec + problem.Avec);
    0.5 * (Svec - Lvec + problem.Avec)
  ]
end

function subgradient_range(
  problem::RobustPCAProblem,
  Lvec::AbstractVector{Float64},
  Svec::AbstractVector{Float64},
)
  m = problem.m
  n = problem.n
  LSdiff = 0.5 * [Lvec + Svec - problem.Avec;
                  Lvec + Svec - problem.Avec]
  ds = norm(LSdiff)
  return (ds ≤ 1e-15) ? zeros(2 * m * n) : LSdiff / ds
end
...
function dist_range(
  ...
  return (1 / sqrt(2)) * norm(Lvec + Svec - problem.Avec)
end

"""
  loss(problem::RobustPCAProblem)

Callable implementing the loss of the set feasibility formulation of robust
PCA.
"""
function loss(problem::RobustPCAProblem)
  m = problem.m
  n = problem.n
  loss_fn(z) = begin
    Lvec = view(z, 1:(m * n))
    Svec = view(z, (m * n + 1):(2 * m * n))
    return dist_range(problem, Lvec, Svec) +
      dist_sparse(problem, Svec) +
      dist_lowrank(problem, Lvec)
  end
  return loss_fn
end

"""
  subgradient(problem::RobustPCAProblem)

Callable implementing a subgradient of the set feasibility formulation of
robust PCA.
"""
function subgradient(problem::RobustPCAProblem)
  m = problem.m
  n = problem.n
  grad_fn(z) = begin
    Lvec = view(z, 1:(m * n))
    Svec = view(z, (m * n + 1):(2 * m * n))
    return [
      subgradient_lowrank(problem, Lvec);
      subgradient_sparse(problem, Svec)
    ] + subgradient_range(problem, Lvec, Svec)
  end
  return grad_fn
end

"""
  proj_sparse_lowrank!(problem::RobustPCAProblem, z::AbstractVector{Float64})
"""
function proj_sparse_lowrank!(
  problem::RobustPCAProblem,
  ...
  Lvec = view(z, 1:(m * n))
  Svec = view(z, (m*n + 1):(2 * m * n))
  Lnew = 0.5 * (Lvec - Svec + problem.Avec)
  Snew = 0.5 * (Svec - Lvec + problem.Avec)
  copyto!(Lvec, proj_lowrank(problem, Lnew))
  copyto!(Svec, proj_sparse!(problem, Snew))
  return z
end

"""
  alternating_projections_step(problem::RobustPCAProblem, z::AbstractVector{Float64})

Callable implementing a step of the alternating projections method applied to
...
function alternating_projections_step(
  ...
  return proj_sparse_lowrank!(problem, z)
end

"""
  initializer(problem::RobustPCAProblem, δ::Float64)

Generate an initial point for a robust PCA `problem` that is `δ`-close to the
ground truth in normalized distance.
"""
function initializer(problem::RobustPCAProblem, δ::Float64)
  ΔL = randn(length(problem.Lvec))
  ΔS = randn(length(problem.Svec))
  return [
    proj_lowrank(problem, problem.Lvec + δ * normalize(ΔL));
    proj_sparse!(problem, Vector(problem.Svec) + δ * normalize(ΔS))
  ]
end
